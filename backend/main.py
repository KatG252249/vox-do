    ...
    try:
        ...
        if data.get('artifact') and data['artifact'].get('type'):
            ...
            if access_token:
                ...
                elif artifact['type'].upper() in ['SLIDE', 'SLIDES', 'PRESENTATION', 'PPT', 'PPTX']:
                    
                    slide_content = artifact.get('content', data.get('journal', {}).get('summary', 'Generated by VoxDo'))
                    
                    slide_url = services.create_google_slide(access_token, title=artifact.get('name', 'VoxDo Presentation'))
                    artifact['url'] = slide_url
                    artifact['action'] = 'open' if slide_url else 'failed'
                    
                elif artifact['type'].upper() == 'FORM':
                    form_url = services.create_google_form(access_token, title=artifact.get('name', 'VoxDo Form'))
                    artifact['url'] = form_url
                    artifact['action'] = 'open' if form_url else 'failed'
